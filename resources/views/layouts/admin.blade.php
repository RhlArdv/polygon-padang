...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased text-slate-800 bg-slate-50" x-data="{ sidebarOpen: false, sidebarCollapsed: localStorage.getItem('sidebarCollapsed') === 'true' }" x-init="$watch('sidebarCollapsed', val => localStorage.setItem('sidebarCollapsed', val))">
        <div class="flex h-screen overflow-hidden">
            ...
            >
                ...
                <div class="flex items-center justify-between h-20 px-4 border-b border-slate-100">
                    <a href="{{ route('dashboard') }}" class="flex items-center gap-3 overflow-hidden whitespace-nowrap">
                        ...
                        <span x-show="!sidebarCollapsed" x-transition.opacity.duration.300ms class="text-xl font-bold tracking-tight text-slate-800">
                ...
                <nav class="flex-1 px-3 py-6 space-y-2 overflow-y-auto overflow-x-hidden">
                    <p x-show="!sidebarCollapsed" x-transition.opacity class="px-3 text-[10px] font-bold tracking-widest text-slate-400 uppercase mb-4">Main Menu</p>
                    
                    <a href="{{ route('dashboard') }}" class="flex items-center gap-3.5 px-3 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('dashboard') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-800 font-medium' }}" title="Dashboard">
                    ...
                    <a href="{{ route('layers.index') }}" class="flex items-center gap-3.5 px-3 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('layers.*') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-800 font-medium' }}" title="Kelola Kategori Layer">
                        <div class="flex-shrink-0 flex items-center justify-center w-6 h-6">
                            <svg class="w-5 h-5 {{ request()->routeIs('layers.*') ? 'text-indigo-600' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
                        </div>
                        <span x-show="!sidebarCollapsed" class="whitespace-nowrap text-sm">Kelola Layer</span>
                    </a>
                    
                    <a href="{{ route('peta.index') }}" class="flex items-center gap-3.5 px-3 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('peta.*') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-800 font-medium' }}" title="Kelola Item Peta">
                        <div class="flex-shrink-0 flex items-center justify-center w-6 h-6">
                ...
                <div class="p-3 border-t border-slate-100">
                    <div class="flex items-center gap-3 p-2 mb-3 rounded-xl hover:bg-slate-50 cursor-pointer overflow-hidden whitespace-nowrap" :class="sidebarCollapsed ? 'justify-center' : ''">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="flex items-center gap-3.5 px-3 py-2.5 text-red-600 hover:bg-red-50 hover:text-red-700 rounded-xl transition-all duration-200 font-medium w-full" :class="sidebarCollapsed ? 'justify-center' : ''" title="Log Out">
                            <div class="flex-shrink-0 flex items-center justify-center w-6 h-6">
            ...
            <div class="flex flex-col flex-1 overflow-hidden h-full">
                ...
                <!-- Top Header -->
                <header class="flex items-center justify-between px-6 py-4 bg-white border-b border-slate-200 sticky top-0 z-10">
                    <div class="flex items-center gap-4">
                        <!-- Mobile Hamburger -->
                        <button @click="sidebarOpen = true" class="lg:hidden text-slate-500 hover:text-indigo-600 focus:outline-none transition-colors">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
                        </button>
                        
                        <!-- Desktop Collapse Toggle -->
                        <button @click="sidebarCollapsed = !sidebarCollapsed" class="hidden lg:flex items-center justify-center w-8 h-8 rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-500 transition-colors">
                            ...
                            <svg x-show="sidebarCollapsed" x-cloak class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 5l7 7-7 7M5 5l7 7-7 7"></path></svg>
                        </button>
